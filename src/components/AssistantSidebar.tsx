...
declare global {
  interface Window {
    electronAPI?: {
      ...
      mcp: {
        runTests: (workspacePath: string) => Promise<{ success: boolean; output: string; error?: string }>;
        createPR: (title: string, description: string, workspacePath: string) => Promise<{ success: boolean; message: string; error?: string }>;
      };
      applyPatch: (patch: Patch) => Promise<{ success: boolean; message: string; error?: string }>;
...
const AssistantSidebar: React.FC<AssistantSidebarProps> = ({ 
  ...
  const handleApplyPatch = async () => {
    // Example patch - in production, this would come from the assistant's response
    const patchText = prompt('Enter patch (JSON format):\nExample: {"file": "src/App.tsx", "edits": [{"start": 0, "end": 10, "replacement": "// New code"}]}');
    if (!patchText) return;

    try {
      const patch = JSON.parse(patchText) as Patch;
